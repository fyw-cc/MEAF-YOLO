...
class MLKCA(nn.Module):
    def __init__(self, c: int):
        super().__init__()
        # 多尺度 DW 分支（可按需调整核/膨胀）
        self.dw7     = nn.Conv2d(c, c, 7, 1, 3, groups=c)
        self.dw9d2   = nn.Conv2d(c, c, 9, 1, 8, dilation=2, groups=c)
        self.dw11d3  = nn.Conv2d(c, c, 11, 1, 15, dilation=3, groups=c)

        # 融合 -> 注意力
        self.fuse_pw = nn.Conv2d(3 * c, c, 1, 1)
        self.bn      = nn.BatchNorm2d(c)
        self.act     = nn.SiLU(inplace=True)
        self.proj_pw = nn.Conv2d(c, c, 1, 1)  # 额外投影增强表达（可关）

    def forward(self, x):
        a = self.dw7(x)
        b = self.dw9d2(x)
        d = self.dw11d3(x)
        attn = self.fuse_pw(torch.cat([a, b, d], dim=1))
        attn = self.act(self.bn(attn))
        attn = torch.sigmoid(self.proj_pw(attn))
        return x * attn

class HFP(nn.Module):
    def __init__(self, c: int, se_ratio: int = 4):
        super().__init__()
        # 低频估计：DW-Blur（5×5）
        self.dw_blur = nn.Conv2d(c, c, 5, 1, 2, groups=c)
        # 高频增强：DW 3×3 + PW
        self.dw3 = nn.Conv2d(c, c, 3, 1, 1, groups=c)
        self.pw  = nn.Conv2d(c, c, 1, 1)
        # SE 门
        ...
        # 初始化 blur 为“近似均值核”（可学习，初始稳定）
        with torch.no_grad():
            k = torch.ones(c, 1, 5, 5) / 25.0
            self.dw_blur.weight.copy_(k)
            if self.dw_blur.bias is not None:
                self.dw_blur.bias.zero_()
