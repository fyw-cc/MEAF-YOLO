...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        m = self.model[-1]
        # 仅当属性存在且为 Tensor 时再迁移，避免 ASFFHead 没有 anchors/strides 报错
        if isinstance(m, (Detect, AMSFHead)):
            if hasattr(m, "stride") and isinstance(getattr(m, "stride"), torch.Tensor):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect):
            ...
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s).to(torch.device("cuda:0")))]
            )
            self.stride = m.stride
            m.bias_init()

        # ✅ 新增：ASFFHead 的 stride 推断与 bias 初始化
        elif isinstance(m, AMSFHead):
            s = 640
            m.inplace = self.inplace
            m.cuda()
            ...
            m.stride = torch.tensor(
                [s / x.shape[-2] for x in _forward_asff(torch.zeros(1, ch, s, s).to(torch.device("cuda:0")))]
            )
            self.stride = m.stride
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
            ...
            if len(f) == 2:
                ...
                args = [ch[f[0]], [ch[i] for i in f[1:]], *args]
            c2 = ch[f[0]]
        ...
        # ✅ 这里加入 ASFFHead，使其获得 [ch[x] for x in f]
        elif m in frozenset(
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, v10Detect, AMSFHead}
        ):
...
def guess_model_task(model):
    def cfg2task(cfg):
        ...
        if m in {"classify", "classifier", "cls", "fc"}:
            return "classify"
        if m in {"detect", "asffhead", "asffhead_v8v10"}:
            return "detect"
        if "segment" in m:
            return "segment"
        if m == "pose":
    ...
    if isinstance(model, torch.nn.Module):
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, WorldDetect, YOLOEDetect, v10Detect, AMSFHead)):
                return "detect"
