...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, WIoU=True, eps=1e-7):
    """
    ...
    # 添加WIOU计算（基于CIoU并增加权重因子）
    if WIoU:
        # 计算CIoU的基础组件（中心距离、宽高比等）
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # 最小外接框宽度
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # 最小外接框高度
        c2 = cw.pow(2) + ch.pow(2) + eps  # 最小外接框对角线平方
        rho2 = ((b2_x1 + b2_x2 - b1_x1 - b1_x2).pow(2) + (b2_y1 + b2_y2 - b1_y1 - b1_y2).pow(2)) / 4  # 中心距离平方
        v = (4 / math.pi ** 2) * ((w2 / h2).atan() - (w1 / h1).atan()).pow(2)  # 宽高比一致性
        with torch.no_grad():
            alpha = v / (v - iou + (1 + eps))  # 宽高比权重

        # WIOU权重因子：聚焦高IoU区域（动态调整权重）
        w = 2 - torch.exp(torch.log(torch.tensor(0.5)) * (iou ** 2))  # 示例权重公式，可根据论文调整
        return iou - w * (rho2 / c2 + v * alpha)  # WIOU = IoU - 加权惩罚项
