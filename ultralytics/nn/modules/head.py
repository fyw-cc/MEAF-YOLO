...
from .conv import Conv, DWConv, MKPConv
from .transformer import MLP, DeformableTransformerDecoder, DeformableTransformerDecoderLayer
from .utils import bias_init_with_prob, linear_init
from typing import List, Tuple


__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect", "YOLOEDetect", "YOLOESegment", "AMSFHead"
...
class DFL(nn.Module):
    def __init__(self, reg_max=16):
        ...
        self.register_buffer("project", torch.linspace(0, reg_max - 1, reg_max).view(1, reg_max, 1, 1))

    def forward(self, x):
        B, C, H, W = x.shape
        assert C == 4 * self.reg_max, f"DFL expect {4*self.reg_max} channels, got {C}"
        x = x.view(B, 4, self.reg_max, H, W)
        x = F.softmax(x, dim=2)
        out = (x * self.project).sum(dim=2)
        return out


class ConvBNAct(nn.Module):
    def __init__(self, c1, c2, k=3, s=1, p=None, g=1, d=1, act=True, bias=False):
        super().__init__()
        if p is None:
            p = ((k - 1) // 2) * d
...
class AMSFBlock(nn.Module):
    """Adaptive multi-scale fusion block used at one target detection level."""

    def __init__(self, in_chs: List[int], out_ch: int, target_level: int):
        super().__init__()
        assert len(in_chs) == 3
        self.in_chs = in_chs
        self.out_ch = out_ch
        ...
        self.lateral = nn.ModuleList([ConvBNAct(c, out_ch, k=1, s=1, p=0) for c in in_chs])
        self.weight = nn.ModuleList([
            nn.Sequential(ConvBNAct(out_ch, out_ch, 3, 1), nn.Conv2d(out_ch, 1, 1))
            for _ in in_chs
        ])
        self.post = ConvBNAct(out_ch, out_ch, 3, 1)

    def forward(self, feats: List[torch.Tensor]) -> torch.Tensor:
        p3, p4, p5 = feats
        maps = [self.lateral[0](p3), self.lateral[1](p4), self.lateral[2](p5)]
        H, W = maps[self.target_level].shape[-2:]
        maps = [F.interpolate(m, size=(H, W), mode="bilinear", align_corners=False) if m.shape[-2:] != (H, W) else m
                for m in maps]
        ws = [self.weight[i](maps[i]) for i in range(3)]
        w = torch.cat(ws, dim=1)
        w = F.softmax(w, dim=1)
        fused = w[:, 0:1] * maps[0] + w[:, 1:2] * maps[1] + w[:, 2:3] * maps[2]
        return self.post(fused)

class AMSFHead(nn.Module):
    """Adaptive Multi-Scale Fusion Head (AMSF-Head in the paper)."""
    ...
    def __init__(self, nc=80, ch: Tuple[int, int, int]=(256, 512, 1024), reg_max=16, head_ch=None):
        super().__init__()
        self.nc = nc
        self.nl = len(ch)
        assert self.nl == 3
        ...
        self.asff = nn.ModuleList([
            AMSFBlock([c2, c3, c4], head_ch, target_level=0),
            AMSFBlock([c2, c3, c4], head_ch, target_level=1),
            AMSFBlock([c2, c3, c4], head_ch, target_level=2),
        ])

        self.reg_branch = nn.ModuleList([
            nn.Sequential(ConvBNAct(head_ch, head_ch, 3), ConvBNAct(head_ch, head_ch, 3), nn.Conv2d(head_ch, 4 * reg_max, 1))
            for _ in range(self.nl)
        ])
        self.cls_branch = nn.ModuleList([
            nn.Sequential(ConvBNAct(head_ch, head_ch, 3), ConvBNAct(head_ch, head_ch, 3), nn.Conv2d(head_ch, nc, 1))
            for _ in range(self.nl)
        ])
    ...
    def bias_init(self):
        for a, b, s in zip(self.reg_branch, self.cls_branch, self.stride if self.stride.numel() else [8, 16, 32]):
            a[-1].bias.data[:] = 1.0
            b[-1].bias.data[: self.nc] = math.log(5 / self.nc / (640 / float(s)) **2)

    def forward(self, feats: List[torch.Tensor]):
        assert len(feats) == 3, "AMSFHead requires three feature maps [P2, P3, P4]"
        B = feats[0].shape[0]

        outs = []
        for i in range(self.nl):
            f = self.asff[i](feats)
            reg = self.reg_branch[i](f)
            cls = self.cls_branch[i](f)
        ...
        shape0 = outs[0].shape
        x_cat = torch.cat([o.view(B, self.no, -1) for o in outs], dim=2)

        if (not self.stride.numel()) or (self.stride.max() == 0):
            self.stride = torch.tensor([4., 8., 16.], device=outs[0].device)
        if self.dynamic or self.shape != shape0:
            anchors, strides = make_anchors(outs, self.stride, 0.5)
            anchors = anchors.to(outs[0].device)
            strides = strides.to(outs[0].device)
            self.anchors = anchors.t().unsqueeze(0)
            self.strides = strides[:, 0].view(1, 1, -1)
            self.shape = shape0

        if self.reg_max > 1:
            box_raw, cls = x_cat.split((self.reg_max * 4, self.nc), dim=1)
            box_raw = box_raw.view(B, 4 * self.reg_max, 1, -1)
            box = self.dfl(box_raw).view(B, 4, -1)
        else:
            box, cls = x_cat[:, :4], x_cat[:, 4:]
            box = box.view(B, 4, -1)

        dbox = dist2bbox(box, self.anchors.to(box.dtype), xywh=True, dim=1) * self.strides.to(box.dtype)
